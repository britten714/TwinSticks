...
public class GameManager : MonoBehaviour
{
    ...
    {
        isPaused = pauseStatus;                 //그 경우 pauseStatus == true가 되서 isPaused 역시 true가 될 것이다. 다만 그렇다고 해서 게임이 멈추지는 않는데 왜냐하면 위에서 게임을 멈추게 하려면 P를 눌러야 한다고 정했으니까. 그럼 어떻게? 여기서는 다루지 않는다. 불완전한 코드. 지금은  이런 내용있다고만 알아두자. 아무튼 API 읽어보면 우리가 게임하다가 홈버튼 눌렀을 때 이 메서드가 작동된다고 함. OnApplicationFocus 도 나중에 함께 공부. 
    }
}
